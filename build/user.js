...
class User {
    ...
    sendMsg() {
        ...
        btnSend.addEventListener("click", () => {
            ...
            if (!inputArea.hasAttribute("data-comment-index")) {
                ...
                app_2.app.data[commentIndex].text = `<div class="comment-block" data-index="${commentIndex}">
                ...
                replyItemsNew.forEach((item) => {
                    ...
                    app_2.app.data[Number(commentIndex)].answers[Number(answerIndex)].text =
                        newText;
                    localStorage.setItem("data", JSON.stringify(app_2.app.data));
                });
    ...
    ratecomment() {
        ...
        counterscomment.forEach((item) => {
            item.addEventListener("click", (e) => {
                const target = e.target;
                let commentIndex = Number(target
                    .closest(".comment-block")
                    .getAttribute("data-index"));
                app_2.app.data[Number(commentIndex)].rate = (target
                ...
                (target
                    .closest(".post-options-counter")
                    .querySelector(".post-options__num")).value = app_2.app.data[Number(commentIndex)].rate;
                localStorage.setItem("data", JSON.stringify(app_2.app.data));
            });
        });
    }
    handlerRate(e) {
        ...
        let commentIndex = Number(target
            .closest(".reply-block__item")
            .getAttribute("data-comment-index"));
        let answerIndex = Number(target
            .closest(".reply-block__item")
            .getAttribute("data-answer-index"));
        app_2.app.data[Number(commentIndex)].answers[Number(answerIndex)].rate = target
            .closest(".post-options-counter")
            .querySelector(".post-options__num").value;
        if (target.closest(".btn-counter--minus")) {
            --app_2.app.data[Number(commentIndex)].answers[Number(answerIndex)].rate;
        ...
        (target
            .closest(".post-options-counter")
            .querySelector(".post-options__num")).value = app_2.app.data[Number(commentIndex)].answers[Number(answerIndex)].rate;
        localStorage.setItem("data", JSON.stringify(app_2.app.data));
    }
    rateAnswer() {
    ...
    sortingByRate() {
        ...
        btnRate.addEventListener("click", function () {
            let sorted = [...items].sort(function (a, b) {
                return a.querySelector(".post-options__num")
                    .value >
                    b.querySelector(".post-options__num").value
                    ? -1
                    : 1;
            });
            commentList.innerHTML = "";
