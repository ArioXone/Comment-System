...
export class User {
  ...
  addUserData() {
    ...
    let avatar = <HTMLElement>(
      document.querySelector(".write-block-area__avatar")
    );
  ...
  sendMsg() {
    let inputArea = <HTMLInputElement>(
      document.querySelector(".send-area__input")
    );
    ...
    btnSend.addEventListener("click", () => {
      ...
      if (!inputArea.hasAttribute("data-comment-index")) {
        ...
        app.data[
          commentIndex
        ].text = `<div class="comment-block" data-index="${commentIndex}">
        ...
        let rateListcomment = document.querySelectorAll(
          ".post-options__num"
        ...
        rateListcomment.forEach((item) => {
          ...
          if (item.hasAttribute("data-num-comment")) {
            let commentIndex: number = Number(
              item.closest(".comment-block")!.getAttribute("data-index")
            );
        ...
        let rateListAnswer = document.querySelectorAll(
          ".post-options__num"
        ) as NodeListOf<HTMLInputElement>;
        ...
        let countercomments = <HTMLElement>(
          document.querySelector(".comment-num")
        );
        ...
        let areacommentIndex: number = Number(
          inputArea.getAttribute("data-comment-index")
        );
        ...
        let answerList = <HTMLElement>document
          .querySelector(".comment-list")!
          .querySelector('[data-index="' + areacommentIndex + '"]')!
          .querySelector(".reply-block");

        answerList.innerHTML += answerTemplate.text;

        app.data[areacommentIndex].answers.push(answerTemplate);

        let replyItems = document
          .querySelector(`[data-index="${areacommentIndex}"]`)!
        ...
        replyItemsNew.forEach((item) => {
          ...
          let originAuthor = <HTMLElement>(
            item
              .closest(".comment-block")!
              .querySelector(".header-post__author")
          );

          let replyAuthor = <HTMLElement>(
            item.querySelector(".header-post__author--reply")!
          );
          ...
          app.data[Number(commentIndex)].answers[Number(answerIndex)].text =
            newText;

          localStorage.setItem("data", JSON.stringify(app.data));
        });

        let rateList = document.querySelectorAll(
          ".post-options__num"
        ) as NodeListOf<HTMLInputElement>;

        rateList.forEach((item) => {
          if (item.hasAttribute("data-num-answer")) {
            let commentIndex = item
              .closest(".reply-block__item")!
              .getAttribute("data-comment-index");

            let answerIndex = item
              .closest(".reply-block__item")!
              .getAttribute("data-answer-index");
  ...
  replyMsg() {
    ...
    btnsReply.forEach((item) => {
      item.addEventListener("click", (e) => {
        ...
        let answercommentIndex = (<HTMLElement>(
          target
        )).parentElement!.parentElement!.parentElement!.parentElement!.parentElement!.parentElement!.getAttribute(
          "data-index"
        );
  ...
  ratecomment() {
    ...
    let counterscomment = document.querySelectorAll(
      "[data-num-comment-block]"
    ) as NodeListOf<HTMLInputElement>;
    ...
    counterscomment.forEach((item) => {
      item.addEventListener("click", (e: Event) => {
        ...
        app.data[Number(commentIndex)].rate = (<HTMLInputElement>(
          (<HTMLElement>target)
        ...
        if ((<HTMLElement>target).closest(".btn-counter--minus")) {
          ...
          if (
            !app.data[commentIndex].rated.find(
              (item: { id: number }) => item.id === currentUser.id
            )
          ) {
        ...
        (<HTMLInputElement>(
          (<HTMLElement>target)
            .closest(".post-options-counter")!
            .querySelector(".post-options__num")
        )).value = app.data[Number(commentIndex)].rate;
  ...
  handlerRate(e: Event) {
    ...
    if ((<HTMLElement>target).closest(".btn-counter--minus")) {
      ...
      if (
        !app.data[commentIndex].answers[answerIndex].rated.find(
          (item: { id: number }) => item.id === currentUser.id
        )
      ) {
      ...
      if (
        app.data[Number(commentIndex)].answers[Number(answerIndex)].rate < 0
      ) {
        (<HTMLElement>target)
          .closest(".post-options-counter")!
          .querySelector(".post-options__num")!
    ...
    (<HTMLInputElement>(
      (<HTMLElement>target)
        .closest(".post-options-counter")!
        .querySelector(".post-options__num")
    )).value = app.data[Number(commentIndex)].answers[Number(answerIndex)].rate;
  ...
  addToFavoritecomment() {
    ...
    let btnsFavoritecomment = document.querySelectorAll(
      ".post-options-favorite-comment"
    );
    ...
    btnsFavoritecomment.forEach((item) => {
      item.addEventListener("click", (e: Event) => {
        ...
        let userIndex: number = app.data[commentIndex].favorite.findIndex(
          (item: { id: number }) => item.id === currentUser.id
        );

        if (
          !app.data[commentIndex].favorite.find(
            (item: { id: number }) => item.id === currentUser.id
          )
        ) {
          app.data[commentIndex].favorite.push(favoriteData),
            ...
            ((<HTMLElement>target)
              .closest(".post-options-favorite-comment")!
              .querySelector(".btn-favorite")!.textContent = "В избранном");
          ...
          app.data[commentIndex].favorite.splice(userIndex, 1),
            ...
            ((<HTMLElement>target)
              .closest(".post-options-favorite-comment")!
              .querySelector(".btn-favorite")!.textContent = "В избранноe");
  ...
  addToFavoriteAnswer() {
    ...
    let btnsFavoriteAnswer = document.querySelectorAll(
      ".post-options-favorite-answer"
    );
    ...
    btnsFavoriteAnswer.forEach((item) => {
      item.addEventListener("click", (e: Event) => {
        ...
        let commentIndex = Number(
          item.closest(".reply-block__item")!.getAttribute("data-comment-index")
        );

        let answerIndex = Number(
          item.closest(".reply-block__item")!.getAttribute("data-answer-index")
        );

        let userIndex: number = app.data[commentIndex].answers[
          answerIndex
        ].favorite.findIndex(
          (item: { id: number }) => item.id === currentUser.id
        );
        ...
        if (
          !app.data[commentIndex].answers[answerIndex].favorite.find(
            (item: { id: number }) => item.id === currentUser.id
          )
        ) {
          app.data[commentIndex].answers[answerIndex].favorite.push(
            favoriteData
          ),
            (<HTMLElement>target)
              .closest(".reply-block__item")!
          ...
          app.data[commentIndex].answers[answerIndex].favorite.splice(
            userIndex,
            1
          ),
            (<HTMLElement>target)
  ...
  sortingByDate() {
    ...
    function defaultSorting() {
      let sorted = [...items].sort(function (a, b) {
        return a.querySelector(".header-post__posted")!.innerHTML >
          b.querySelector(".header-post__posted")!.innerHTML
          ? -1
          : 1;
      });
  ...
  sortingByRate() {
    ...
    btnRate.addEventListener("click", function () {
      let sorted = [...items].sort(function (a, b) {
        return (<HTMLInputElement>a.querySelector(".post-options__num"))!
          .value >
          (<HTMLInputElement>b.querySelector(".post-options__num"))!.value
          ? -1
          : 1;
      });
  ...
  sortingByAnswersQuantity() {
    ...
    btnAnswers.addEventListener("click", function () {
      let sorted = [...items].sort(function (a, b) {
        return a.querySelectorAll(".reply-block__item").length >
          b.querySelectorAll(".reply-block__item").length
          ? -1
          : 1;
      });
